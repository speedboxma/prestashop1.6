...
class Speedbox extends CarrierModule
{
    ...
    public function __construct()
    {
        ...
        $this->description = $this->l('Module de livraison pour Prestashop 1.6');
    ...
    public function install()
    {
        ...
        $carrier = $this->addCarrier("Service de Livraison SpeedBox");
    ...
    public function getOrderShippingCost($params, $shipping_cost)
    {
        if (Context::getContext()->customer->logged == true) {
            ...
            $cost = $this->calculateShippingCost($address->city); // a overrider
    ...
    protected function addCarrier($carrier_name)
    {
        $carrier = new Carrier();
        ...
        foreach (Language::getLanguages() as $lang) {
            $carrier->delay[$lang['id_lang']] = $this->l('Service de livraison au Maroc');
        }
...
// ajouté via template
    ...
    public function hookHeader()
    {
        ...
        $this->context->smarty->assign(array(
            ...
            'selectedCity'               => (!empty($address_details['city'])) ? $address_details['city'] : '',
    ...
    public function hookNewOrder($params)
    {
        ...
        if ($params['order']->id_carrier == Configuration::get('SPEEDBOX_RELAIS_CARRIER')) {
            ...
            $type_livraison = Context::getContext()->cookie->speedbox_type_livraison;
            ...
            //else {
                //$adress_infos = $order->id_address_delivery;
                //$address = new Address($address_infos);
                //$this->speedbox_orders_model->SetSpeedboxAttributeOrder($order->id, '   speedbox_selected_relais_infos', getAddressDetails());
            //}
            $this->speedbox_orders_model->SetSpeedboxAttributeOrder($order->id, 'speedbox_statut_colis', '-');
            $this->speedbox_orders_model->SetSpeedboxAttributeOrder($order->id, 
                'speedbox_type_livraison',$type_livraison);

            $order->speedbox_selected_relais_id    = $relay_id;
            $order->speedbox_selected_relais_infos = $relay_info;
            $order->speedbox_type_livraison        = $type_livraison;
            $order->update();
